...
sequenceDiagram
    ...
    API->>DB: INSERT INTO instance_reports<br/>(instance_id, batch_id, label, n8n_version, data, received_at)
    Note over DB: Append-only event store.<br/>dataPoints stored as a JSON blob.<br/>UNIQUE (instance_id, batch_id).
