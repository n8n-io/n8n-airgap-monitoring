...
Billable usage (e.g. `billableExecutions`) is reported as a `DailyMetric`: one value per UTC calendar day, tagged with a `batchId` for retry/duplicate-instance handling (see `src/usage/usage.service.ts`). Scoping each report to a single day bounds the damage of a customer-side DB rollback to the affected days, instead of corrupting a lifetime counter the way a pure running total would.
