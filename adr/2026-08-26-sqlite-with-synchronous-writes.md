...
- **A future read path is the larger risk, not the write.** `adr/2026-08-18-store-datapoints-as-json.md` contemplates fleet-wide reconciliation scans over `json_each()` costing *seconds*. WAL means such a reader does not block the writer at the database level, but a synchronous seconds-long scan on the event loop stalls every request in the process. Any reporting UI or reconciliation job must therefore keep synchronous work off the event loop, and that requirement follows from this ADR rather than from theirs. **`adr/2026-09-04-stream-the-usage-report.md` narrows the remedy this bullet originally assumed.** It anticipated a separate process or a worker thread; the report export instead stays in the request path by splitting the read into short per-instance queries and yielding to the event loop between them, measured at ~1ms per chunk. The constraint stands — a seconds-long synchronous scan is still disqualifying — but "outside the request path" turned out to be one remedy rather than the requirement.
...
- Tests run against a throwaway file under the OS temp directory (`src/testing/build-app.ts`), so they take the same code path as production, WAL included. They still do not *assert* on the pragmas: a change to `journal_mode` or `synchronous` would not fail a test, and the durability behaviour this ADR decides on remains unverified by CI.
