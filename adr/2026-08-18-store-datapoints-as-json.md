...
- Metric-level semantics live in read-side code, not in the database: `batchId` dedup, the daily-vs-cumulative rules, and value typing are enforced only by Ajv at the HTTP boundary and by whatever queries consume the rows. Consumers should share one query helper or SQL view built on `json_each()` so the dedup rule is written once, and any second write path (backfill, manual adjustment) must reuse the same validation, because the database will accept anything.
