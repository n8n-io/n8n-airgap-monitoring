 ...
 * /api/v1/ingest endpoint, so seeded data goes through the same validation
 * and business logic as production traffic instead of writing to SQLite
 * directly.
 *
 * Usage: start the app (`pnpm dev` or `pnpm start`), then in another shell:
 *   N8N_AUTH_TOKEN=<token> pnpm seed
 * Optional: N8N_BASE_URL (default http://localhost:3000)
 */
import { createHash } from 'node:crypto'
import type { Metric, UsageReport } from '../src/usage/usage.service'
...
async function seed (): Promise<void> {
  if (authToken === undefined || authToken.trim() === '') {
    console.error('N8N_AUTH_TOKEN must be set to the token the target instance was started with')
    process.exitCode = 1
    return
  }
