...
Doubling the request ceiling of the HTTP layer serves the hosting requirement: it widens the margin before a deployment we cannot inspect becomes a support ticket we cannot reproduce. It is the router's ceiling, not the service's — see the note in Consequences.
